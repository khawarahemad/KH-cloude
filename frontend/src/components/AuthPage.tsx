...
export default function AuthPage({ onBack, onAuthSuccess }: AuthPageProps) {
  ...
  const handleOAuthSimulate = (provider: string) => {
    ...
    if (provider === 'Google') {
      const clientId = process.env.NEXT_PUBLIC_GOOGLE_CLIENT_ID || '1017835150826-placeholder.apps.googleusercontent.com';
      ...
      window.location.href = `https://accounts.google.com/o/oauth2/v2/auth?client_id=${clientId}&redirect_uri=${encodeURIComponent(redirectUri)}&response_type=code&scope=openid%20profile%20email&state=${state}`;
      return;
    }
