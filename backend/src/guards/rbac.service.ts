...
export class RbacService {
   ...
   *
   * @param userId  - Comes from req.user.id (set by JwtAuthGuard). Never from request headers/body.
  ...
  async verifyProjectAccess(userId: string, projectId: string, minRole: TeamRole = 'VIEWER') {
  ...
  async verifyDeploymentAccess(userId: string, deploymentId: string, minRole: TeamRole = 'VIEWER') {
  ...
  async verifyDatabaseAccess(userId: string, databaseId: string, minRole: TeamRole = 'VIEWER') {
  ...
  async verifyBucketAccess(userId: string, bucketId: string, minRole: TeamRole = 'VIEWER') {
  ...
  async verifyEdgeFunctionAccess(userId: string, functionId: string, minRole: TeamRole = 'VIEWER') {
    const fn = await this.prisma.edgeFunction.findUnique({
      where: { id: functionId },
      include: { team: true },
    });
    if (!fn) throw new NotFoundException('Edge function not found.');
    const auth = await this.verifyTeamRole(userId, fn.teamId, minRole);
